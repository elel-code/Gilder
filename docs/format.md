...
- 2D `image`、`color`、`rectangle`、`ellipse` 和 `group` 图层。
...
- 当前核心层会解析、校验并可确定性求值 scene graph；daemon 会生成
  ...
  SVG surface，也会把 `rectangle`/`ellipse` shape 合成为 SVG；缺少可合成 layer 时才回退到
  `fallback`、首个 image layer 或首个 color layer。原生动画 scene-lite surface 后续实现。
