...
- 2D `image`、`color` 和 `group` 图层。
- 变换、透明度、keyframe timeline 和动画曲线。
- 用户属性到图层属性的 binding schema。
- 基础粒子或 shader 需要逐项白名单。
- 不执行 SceneScript。
- 当前核心层会解析、校验并可确定性求值 scene graph；daemon 会生成
  `render_sync.scene_lite_plans`，并把 scene fallback 和 image layer 纳入计划层资源
  footprint。GTK renderer 当前显示 `fallback`、首个 image layer 或首个 color layer；
  原生动画 scene-lite surface 后续实现。
