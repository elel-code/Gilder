...
启用 native Vulkan/GStreamer video feature 时，GStreamer 前端会把该上限应用到视频
pipeline；最终显示仍由 native Vulkan importer/render/present 路径负责。
...
native Vulkan runtime 后续负责可见切换和 crossfade。历史 GTK Picture/Stack 路线只作为
已退休实现记录，不再是当前架构目标。
