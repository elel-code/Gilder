...
启用 `[adaptive]` 或单个 `[outputs.<name>.adaptive]` 时，`telemetry.adaptive.snapshot` 会报告 Linux PSI CPU/内存压力、thermal zone 最高温度、power_supply AC/电池细节、DRM `gpu_busy_percent` 统计、`active_triggers` 和 kill switch 状态，`telemetry.adaptive.action` 会列出实际被降载的输出及动作。默认 `action = "throttle"` 会报告 `type: "throttle"` 和 `max_fps`；`action = "pause-unfocused"` 在输出非焦点时报告 `type: "pause-unfocused"` 并暂停该输出，输出仍聚焦或缺少桌面输出状态时回退为 throttle，并在 action report 中保留 `configured_action`。adaptive 决策原因是 `adaptive`；用户暂停、fullscreen pause、battery pause 等更强策略不会被覆盖。
