...
`[performance].battery` 和 `[outputs.<name>.performance].battery` 支持 `continue`、`throttle`、`pause` 和 `pause-dynamic`；`fullscreen` 和 `unfocused` 支持同一组值；`hidden` 和 `session` 支持 `continue`、`pause` 和 `pause-dynamic`。其中 `pause-dynamic` 会等待 manifest 加载完成，只对 video/slideshow/web 生成 `paused`/`remove` 决策，静态壁纸保持原有桌面状态决策。
