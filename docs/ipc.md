...
`telemetry` 会报告桌面快照刷新次数、read 请求复用快照次数、桌面变化次数，以及 `render_sync` 缓存 hit/miss；`--telemetry-csv` 会输出 `desktop_refreshes,desktop_refresh_skips,desktop_changes,last_desktop_refresh_age_ms,render_sync_cache_hits,render_sync_cache_misses`，便于确认状态栏轮询和性能采样没有持续触发 compositor 适配器或重复生成渲染计划。
`scripts/performance-snapshot.sh` 可以用 `--expect-render-sync-cache-hit` 和 `--expect-desktop-refresh-skip` 把这些 telemetry 变成失败条件，适合 CI 或真实会话 smoke 证明缓存和刷新节流仍然生效。
