...
`scene_lite_plans` 包含 scene-lite 文档路径、fallback、time=0 snapshot layer、当前可显示 surface 和桌面状态策略合成后的 `target_max_fps`；当前 GTK renderer 会显示 fallback、首个 image layer 或首个 color layer，真正的动画 scene surface 后续实现。
