...
视频壁纸有 poster 时，`video_plans[].poster` 会保留 fallback 路径；未启用 `video-renderer` 的构建会在 `plans` 里生成同一输出的静态 poster 占位计划。启用 `video-renderer` 时，active 视频不再预创建 poster 静态计划，GTK 会在 pipeline 构建或运行失败时才懒加载 poster fallback。
...
`[performance].battery` 和 `[outputs.<name>.performance].battery` 支持 `continue`、`throttle`、`pause` 和 `pause-dynamic`；`fullscreen` 和 `unfocused` 支持同一组值；`hidden` 和 `session` 支持 `continue`、`pause` 和 `pause-dynamic`。其中 `pause-dynamic` 会等待 manifest 加载完成，只对 video/slideshow/web/scene-lite 生成 `paused`/`remove` 决策，静态壁纸保持原有桌面状态决策。
`slideshow_plans` 包含源图列表、切换间隔、transition、fit 和桌面状态策略合成后的 `target_max_fps`；当前 GTK renderer 会按间隔切换图片，`crossfade` 先作为格式字段保留。
`scene_lite_plans` 包含 scene-lite 文档路径、fallback、time=0 snapshot layer、当前可显示 surface 和桌面状态策略合成后的 `target_max_fps`；当前 GTK renderer 会优先显示合成后的静态 snapshot SVG surface，缺少可合成 layer 时再显示 fallback、首个 image layer 或首个 color layer，真正的动画 scene surface 后续实现。
...
静态 GTK surface 还可以用 `--expect-renderer-static-picture-surfaces-*-at-most`、`--expect-renderer-static-css-surfaces-*-at-most`、`--expect-renderer-static-color-surfaces-*-at-most` 和 `--expect-renderer-static-surface-estimated-decoded-bytes-*-at-most` 约束 Picture/CSS fallback/color 路径数量和估算 decoded footprint；`*` 当前支持 `latest` 或 `max`，适合给静态图纹理保留风险设置预算门槛。`desktop-policy-smoke.sh` 会向每个 headless 场景转发这些预算，`wayland-baseline-matrix.sh` 和 `wayland-video-surface-smoke.sh` 也会把对应 summary key 写入矩阵/报告，便于持续观察 static Picture、tile CSS fallback、color surface 和 crossfade 短时 decoded footprint。
启用 `[adaptive]` 或单个 `[outputs.<name>.adaptive]` 时，`telemetry.adaptive.snapshot` 会报告 Linux PSI CPU/内存压力、thermal zone 最高温度、power_supply AC/电池细节、DRM `gpu_busy_percent` 统计、`active_triggers` 和 kill switch 状态。触发项覆盖 CPU pressure、memory pressure、temperature、GPU busy 和放电时低电量；阈值为 0 可关闭单项触发。`telemetry.adaptive.action` 会列出当前 adaptive 动作。默认 `action = "throttle"` 会报告 `type: "throttle"` 和 `max_fps`；`action = "pause-unfocused"` 在输出非焦点时报告 `type: "pause-unfocused"` 并暂停该输出，输出仍聚焦或缺少桌面输出状态时回退为 throttle，并在 action report 中保留 `configured_action`；`action = "pause-dynamic"` 会报告 `type: "pause-dynamic"` 和 `scope: "dynamic-wallpapers"`，渲染计划加载 manifest 后只暂停 video/slideshow/web/scene-lite，静态图片保持原有桌面状态决策。adaptive 决策原因是 `adaptive`；用户暂停、fullscreen pause、battery pause 等更强策略不会被覆盖。
`telemetry.renderer` 会报告当前 GTK renderer 持有的 output window、静态 background surface、slideshow surface、video surface 数和共享 video runtime 数，以及当前 static surface、slideshow surface 和 video pipeline 引用的源文件引用数、去重资源数和源文件字节 footprint；后者是 GTK renderer 仍持有大图/幻灯片/视频源资源的线索，不是解码纹理内存、GStreamer buffer 或 USS。普通静态图 surface 现在优先是 `gtk::Picture`，`tile` fit 仍会退回 CSS background fallback，颜色 surface 只有背景 provider 而没有源文件引用；`static_picture_surfaces`、`static_css_surfaces`、`static_color_surfaces` 会拆分这些路径，`static_surface_estimated_decoded_bytes` 会按已存在 paintable 的 intrinsic size 估算 RGBA decoded footprint，未知尺寸和 CSS fallback 记为 0。这是保留纹理风险线索，不是实际 GSK texture memory 或 USS。它也会把当前 `renderer_runtime.video_pipelines[].frame_stats` 聚合为视频 pipeline 数、QoS 消息总数、最大 dropped 计数、GTK frame clock tick 总数、before-paint/update/layout/paint/after-paint phase tick 总数、最大 frame interval、最大观察 FPS、GDK frame timings 完成数和 presentation interval/time 线索。这是 daemon 级采样入口，方便状态栏、性能脚本和 smoke 快速判断输出资源是否释放、视频帧行为是否在变化；默认 GTK 视频 frame-clock observer 只记录 after-paint tick、frame counter/time 和 interval，以减少 4K/高刷视频主线程诊断开销；需要 before/update/layout/paint phase、FPS/refresh_info 或 GDK `FrameTimings` 时，启动 daemon 前设置 `GILDER_GTK_VIDEO_FRAME_STATS=full`。逐输出诊断仍应查看 `renderer_runtime.video_pipelines` 或 `--video-runtime-csv`。GTK active video 成功路径不会预创建 poster/static surface，因此 `renderer.static_surfaces` 的最新值应为 0；pipeline 构建失败或后续报错时才会按 `video_plans[].poster` 懒加载 fallback surface。
GTK renderer tick 会按当前负载动态调度：video runtime 单独存在时使用 250ms 常规 polling 来处理 bus/EOS/error/QoS，frame stats 继续按 500ms 写回最近的 runtime snapshot；如果同一进程还有 slideshow 过渡需要更短 tick，则按 slideshow 的短间隔运行；纯静态无动态工作时不安装 renderer runtime timeout，render sync 由 bridge 线程投递 GLib idle wakeup 后在 GTK 主线程立即消费。renderer runtime snapshot 只会在 render sync 变化、slideshow 实际换帧、decoder 观察结果变化或 pipeline 报错时完整刷新；frame stats 到期判断只读取 video runtime 计数，不会重新计算 surface/source footprint；同一次 snapshot 内，共享 video runtime 的 decoder/caps/allocation、position 和 duration 查询只做一次，再展开为逐输出 `video_pipelines`；GTK 组合路径复用 `resource_snapshot()` 里已经计算出的 video source footprint，不会为了序列化 pipeline telemetry 再重复读取源文件 metadata；resource footprint 内部按路径缓存 source size，重复引用仍按引用次数累计 bytes，但每个唯一路径只读取一次 metadata。纯静态空闲输出会继续保留最近一次 snapshot，避免状态栏轮询让 daemon 高频重算 GTK resource footprint。
...
启用 `gtk-renderer` feature 的 daemon 会在 GTK 主线程消费同一份 `render_sync`，并把可用输出同步到 layer-shell background 窗口。GTK 主线程会由 GLib idle wakeup 消费 renderer queue，并 drain 当前积压队列、只应用最新 render sync，避免完整目标状态的中间计划反复创建 GTK/GStreamer surface、pipeline 或 runtime snapshot；video/slideshow 出现后才安装 runtime timeout，纯静态路径不靠周期 renderer tick 轮询 queue。
