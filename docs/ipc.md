...
启用 `gtk-renderer` feature 的 daemon 会在 GTK 主线程消费同一份 `render_sync`，并把可用输出同步到 layer-shell background 窗口。GTK 主线程会 drain 当前积压的 renderer queue 并只应用最新 render sync，避免完整目标状态的中间计划反复创建 GTK/GStreamer surface、pipeline 或 runtime snapshot。
