...
`telemetry.render_sync.package_cache_max_retained_unique_resource_bytes` 会在 JSON status/watch 中报告当前临时 package cache 的去重源资源 footprint 上限；CSV 仍使用 retained resource byte 字段观察实际结果。
