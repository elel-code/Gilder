...
GTK renderer tick 会按当前负载动态调度：存在 video runtime 时保持 50ms，以便更新 bus/QoS/frame-clock/position 证据；纯静态空闲或长间隔 slideshow 会退到最长 250ms。renderer runtime snapshot 只会在 render sync 变化、slideshow 实际换帧或存在 video runtime 时刷新；纯静态空闲输出会继续保留最近一次 snapshot，避免状态栏轮询让 daemon 高频重算 GTK resource footprint。
...
这些 decoder/caps/allocation/memory path 诊断是慢变运行时证据，renderer 内部会按 video runtime 缓存并低频刷新；`frame_stats`、播放位置和 duration 仍会随 runtime snapshot 更新。这样状态栏轮询或 GTK 50ms tick 不会持续触发 GStreamer pipeline 遍历和 allocation query。
