...
GTK renderer 只会在 render sync 变化、slideshow 实际换帧或存在 video runtime 时刷新 renderer runtime snapshot；纯静态空闲输出会继续保留最近一次 snapshot，避免状态栏轮询让 daemon 每 50ms 重算 GTK resource footprint。
