...
`[performance].battery` 和 `[outputs.<name>.performance].battery` 支持 `continue`、`throttle`、`pause` 和 `pause-dynamic`。其中 `pause-dynamic` 会在电池供电时等待 manifest 加载完成，只对 video/slideshow 生成 `paused`/`remove` 决策，静态壁纸保持原有桌面状态决策。
