...
2. Full scene wallpaper support: the current completed work is a first-class
   ...
   is roughly `96%`: package/conversion boundaries, `scene/gscene` format
   ...
   flattening, stroke geometry, deterministic text glyph geometry,
   ...
   shaping/atlas typography, SVG arc/remaining path command coverage, and actual mixed
   ...
   compatibility path. Path layers now parse `M/L/H/V/Z` plus `C/S/Q/T`
   cubic, smooth cubic, quadratic, and smooth quadratic Bezier commands,
   flatten curves into deterministic 16-segment polylines, and feed the
   existing native fill/stroke tessellation path. Common Wallpaper Engine/SVG
   curve shapes therefore render as solid Vulkan geometry instead of staying
   as unsupported path metadata.
   ...
   `curve_path_layer_count`, `text_geometry_layer_count`, and
   ...
   `target_runtime=native-vulkan-full-scene`,
   `current_runtime=native-vulkan-scene-runtime`,
   `progress_estimate_percent=96`, `native_present_route_ready`,
   ...
   `runtime.full_scene.progress_estimate_percent=96`,
   ...
   `scene-we-spritesheet-atlas-runtime`, and
   `curve-path-flattening-runtime`; Hyprland/override cursor scene coverage
   also asserts `cursor-parallax-input-source` completion.
   Next gates:
   wiring mixed video-as-scene layer composition from this explicit bridge boundary,
   complex font shaping/atlas typography, SVG arc/remaining path command coverage,
