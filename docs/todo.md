...
- [ ] 验证 H.265 direct decode output image 内容：把 decode 后的 NV12 array layer 0 接到现有 native Vulkan NV12 shader sampling 或临时 GPU readback/hash，确认像素正确性后再进入连续帧/显示路径。
