...
- [x] 退役旧 ash `probe-video-session` 链路：它曾证明 session/resource/bitstream、codec 参数集转换、首帧/ready-prefix submit、host readback 和离屏 NV12/P010 sampling；当前这些约束转入 parser/reference/Vulkanalia tests，不再保留旧 CLI 或旧 smoke 脚本。
- [x] 将 H.264/H.265/AV1 session/resource/submit gate 收敛到 Vulkanalia：当前入口是 `--probe-vulkanalia-video-session`、H.264/H.265 Vulkanalia streaming video smoke 和 AV1 decode-frame submit lowering，FFmpeg 是 codec/DPB/submit order 第一参考，GStreamer 只作为 demux/parser/audio frontend。
- [x] 历史中间阶段曾将 H.264/H.265 visible direct smoke 和 CLI 默认输入切到 streaming queue，并停止维护 ready-prefix spool 入口：当时可见 video runtime 不再保留长 AU payload，剩余 H.264 4K/240 缺口主要落在 codec capability、present pacing/同步或驱动路径，不是 packet retention。当前这些 CLI/input 模式已退休，统一到 Vulkanalia bounded streaming packet queue + persistent mapped bitstream buffer 路线。
- [x] 为 H.264 streaming planner 覆盖 P/B、多参考、短期 L0/L1 modification、MMCO、long-term、non-existing refs 和 PicNum wrap；剩余 H.264 direct 边界是真实 long-term coded stream、field/interlaced、任意入口 DPB 重建和 4K/240 pacing/同步。
...
- [x] 历史中间阶段曾将可见 H.265 ready-prefix 的 AU payload 保留从 `Vec<Vec<u8>>` 改为临时 spool 文件加单个复用上传 buffer，避免 4K/240 长 ready-prefix 把约 499MB encoded AU payload 计入进程私有内存；spool 已退休，不再维护。2026-06-21 真实 Wayland `HDMI-A-1`、3840x2160@240、4800 AU/4800 playback frames 证据 `/tmp/gilder-vulkan-h265-memory-spooled.d8pybb`：`decoded_frame_count=4800`、`presented_frame_count=4800`、`average_present_fps=239.977`、`bitstream_window_payload_bytes=499056595`、`bitstream_buffer_slot_bytes=249344`，86 个 250ms smaps samples 中 `RSS/PSS/USS/Private_Dirty max=117732/85864/68248/37664 KiB`。旧 in-memory payload 证据 `/tmp/gilder-vulkan-h265-memory.GIYC3r` 为 `RSS/PSS/USS/Private_Dirty max=1089060/1069592/1061636/1008992 KiB`，确认此前高 `Private_Dirty` 主因是 retained AU payload，不是 Vulkan Video resource/session/bitstream 显式资源。
- [x] 历史中间阶段优化 H.265 spool -> mapped bitstream upload：播放循环不再先读入临时 `Vec<u8>` 再拷贝到 Vulkan mapped buffer，而是从 spool file 直接 `read_exact` 到持久映射的 `VIDEO_DECODE_SRC_KHR` buffer；同时跟踪顺序读取位置，顺序帧避免重复 seek，并清理当前 AU aligned range 内的 padding，避免 stale bitstream tail 进入 decoder。该路线已退休，不再维护 spool。
- [x] 历史中间阶段曾将 visible H.265 encoded input 改为 Bitstream Ring Buffer：默认 2-slot、固定容量 `VIDEO_DECODE_SRC_KHR` buffer，按 driver offset/size alignment 追加写入，runtime JSON 记录 `src_buffer_offset`、payload/range、allocation index、wrap count 和 ring capacity；该 ring input 已退休，当前使用 single persistent mapped upload buffer + per-frame streaming packet queue 模型。2026-06-21 真实 Wayland `HDMI-A-1`、3840x2160@240、4800 frames 证据 `/tmp/gilder-vulkan-h265-ready-prefix-video.Ldh5wL`：`average_present_fps=240.041`、`bitstream_buffer_strategy=fixed-capacity-persistent-mapped-ring`、`bitstream_buffer_bytes=498688`、`bitstream_ring_wrap_count=1200`；同配置 smaps 证据 `/tmp/gilder-vulkan-h265-ring-memory.9RFFoa`：`RSS/PSS/USS/Private_Dirty max=117836/86018/68380/37932 KiB`、`average_present_fps=240.063`。
- [x] 历史中间阶段曾将 visible H.265 encoded input 接到 bounded streaming packet queue：当时新增 `NativeVulkanH265VideoInputMode`、CLI `--h265-input streaming-queue` 和脚本 `--streaming-queue`，H.265 按需从 GStreamer parser/appsink 拉 AU、上传到 bitstream ring 后释放 payload，并在 runtime JSON 中报告 `h265_packet_queue_*`。这些入口已随 Vulkanalia unified ready-prefix 主线退休，当前 smoke 使用 `--run-vulkanalia-ready-prefix-video`，并强制 `bounded-streaming-packet-queue-per-frame-upload` + `streaming-persistent-mapped-reused-upload-buffer`。2026-06-21 真实 Wayland `HDMI-A-1` 4K/240 smoke `/tmp/gilder-vulkan-h265-ready-prefix-video.uMgUWp` 为 `decoded/presented=240/240`、`average_present_fps=238.316`、`queue_capacity=32`、`queue_pulled=240`、`queue_retained=0`；20s smaps 证据 `/tmp/gilder-vulkan-h265-streaming-smaps.wTY7vB` 为 `decoded/presented=4800/4800`、`average_present_fps=240.027`、`queue_eos/loops=19/19`、`RSS/PSS/USS/Private_Dirty max=115480/71078/51800/33892 KiB`、平均 CPU `20.11%`。
- [x] 历史中间阶段曾将 H.264/H.265 visible direct streaming packet queue 合并为共用输入层：新增 codec hook/泛型队列，统一 GStreamer pipeline/appsink/bus、bootstrap 参数集选择、EOS loop、payload retained accounting 和 bitstream ring sizing；旧 queue/ring CLI 已退休，当前保留的是 codec parser/snapshot 能力和 Vulkanalia per-frame streaming upload runtime。2026-06-21 真实 Wayland `HDMI-A-1` 回归：H.264 720p/60 `/tmp/gilder-vulkan-common-queue-h264-720p60` 为 `decoded/presented=60/60`、`queue_retained=0`、`average_present_fps=247.329`；H.264 4K/240 `/tmp/gilder-vulkan-common-queue-h264-4k240` 为 `decoded/presented=240/240`、`b_frames=119`、`queue_retained=0`、`average_present_fps=199.922`；H.265 4K/240 `/tmp/gilder-vulkan-common-queue-h265-4k240` 为 `decoded/presented=240/240`、`queue_retained=0`、`average_present_fps=238.368`。H.264 4K/240 长跑 `/tmp/gilder-vulkan-common-queue-h264-4k240-memory/performance` 为 `decoded/presented=2400/2400`、`average_present_fps=204.900`、`queue_eos/loops=9/9`、`queue_retained=0`，8 个 samples 中 `RSS/PSS/USS/Private_Dirty max=105404/63269/45272/26844 KiB`、平均 CPU `12.49%`、NVIDIA 进程 GPU memory `104 MiB`。
...
- [x] 删除 AV1 legacy 可见 smoke 兼容面：real-source matrix 对 AV1 playback 标记为 `skip-av1-streaming-runtime-missing`；后续必须实现 heap-only continuous AV1 streaming runtime。
...
- [ ] 继续推进 audio/clock 主线路：H.264/H.265 streaming runtime 已共用
  ...
  `--pacing-master audio` 接入 H.264/H.265 streaming runtime 和 AV1 interim visible runtime：默认仍保持 target-fps
  ...
  H.264/H.265 streaming smoke 已接入 `--muted/--unmuted`、`--audio-output`
...
- [x] 修正 plan-following audio runtime 的 muted 分支：`audio_policy.rs` 一直将 muted
  解析为 `clock-only`，但 renderer plan runtime 之前只在 `auto` 时启动 worker；现在 muted
...
- [x] 增强 FFmpeg PacketQueue/serial 对齐证据：`demux.rs` 现在区分 loop 边界的配置跳过窗口
  ...
  leading 包”，也能验证实际 source serial 切换后旧包是否被丢弃到 recovery/random-access 边界。
...
- [x] 退役首帧 H.265 ash direct visible smoke：旧首帧入口只覆盖静态
  IDR 重复 present；当前主线改为 Vulkanalia ready-prefix sequence。
- [ ] 将 visible direct H.265 从首帧推进到 ready-prefix sequence：复用已有 DPB/POC 计划，按 AU
  ...
  到 swapchain。
- [x] 接通可见 direct H.265 ready-prefix sequence：当前统一入口为 `--run-vulkanalia-ready-prefix-video --video-codec h265`，历史 `--run-h265-ready-prefix-video` 已退休；该路径复用
...
- [x] 接通可见 direct H.264 ready-prefix sequence：当前统一入口为 `--run-vulkanalia-ready-prefix-video --video-codec h264-high-8`，历史 `--run-h264-ready-prefix-video` 已退休；该路径复用 H.264
...
- [ ] 将 AV1 Main8/Main10 推进到 direct Vulkan Video 任意入口连续可见：下一步必须实现
  continuous streaming runtime，以 GStreamer/后续 FFmpeg frontend 只负责
  ...
  `scripts/native-vulkan-ready-prefix-video-common.sh`，当前 H.264/H.265 smoke 复用
...
- [x] 将 `vulkanalia` 提升为 native Vulkan 主线：`native-vulkan-renderer`
  ...
  同设备 extension/capability/import telemetry 证明。Vulkan 1.2/1.3/1.4 core
  feature/property 探测已集中到 `vulkanalia_backend/features.rs`，device probe、
...
- [x] 将 Vulkanalia video-present session gate 从一次性 probe 推进到 retained runtime
  ...
  family `[3]` + graphics/present family `[0]`。H.265 ready-prefix decode 已接入这组
  retained image/session；下一步是把 dynamic-rendering fullscreen draw 接进 swapchain
  present，而不是继续停留在 clear placeholder。
...
- [x] 将 Vulkanalia H.265 retained decoded image 推进到 graphics sampling 资源边界：
  `vulkanalia_backend/render_present.rs` 在 video+present logical device 上使用
  `VK_EXT_descriptor_heap`，为 retained NV12/P010 image 写入 Y/UV plane sampled-image
  descriptors 和普通 sampler descriptors；同时记录 split queue family 下的
  sync2 release/acquire + dynamic rendering draw 顺序。当前主线不使用 descriptor set、
  push descriptor 或 embedded YCbCr sampler fallback。真实 Wayland `background`
  证据应以 `decoded-image-plane-sampler-present-resource`、NV12/P010 picture format 和
  `retained_submitted=240` 为准。
  该项仍不把 `decoded_image_zero_copy_presented` 置 true；下一 gate 是真正录制
  dynamic-rendering fullscreen draw 并替换 clear placeholder。
...
- [x] 统一 direct 路线 decoded-frame zero-copy evidence：H.264/H.265 streaming runtime 和 AV1 decode-frame probes
  runtime 输出 `decoded_frame_zero_copy_scope/status`，把 direct-DPB/display-copy 证据和
  bitstream-ring upload copy 作用域明确分开。
- [x] 强化 H.265 direct-DPB evidence：H.265 streaming runtime 现在与 H.264/AV1
...
- [x] 继续拆出 direct codec runtime snapshot 类型：`direct_h264_runtime.rs` 承载
  H.264 ready-prefix runtime/frame snapshot。
  `direct_av1_runtime.rs` 承载 AV1
  decode-frame runtime/frame snapshot 和 optional present timing apply impl。三类 direct
...
- [x] 建立 scene-lite sampled image 的 Vulkanalia upload/descriptor 规划边界：
  ...
  descriptor heap 数量、sampled-image alpha-blend pipeline label、`cmd_copy_buffer_to_image`/
  `cmd_bind_scene_lite_descriptor_heap`/`cmd_draw_indexed_per_image_quad` 顺序，并删除
  descriptor set / push descriptor fallback 策略。2026-06-25 继续把 sampled-image draw-pass
  ...
  descriptor-heap mapping、pipeline layout、`PipelineRenderingCreateInfo` graphics pipeline，
