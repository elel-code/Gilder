...
- [ ] 继续推进 audio/clock 主线路：H.264/H.265/AV1 ready-prefix runtime 已共用
  ...
  默认仍是 `clock-only`，避免壁纸静音策略回退。短 H.264 gate
  ...
  `audio_decoders=["avdec_aac"]`、`video_decoders=[]`、`audio_reached_clocked_playback=true`。
  下一步是把该 opt-in 输出接入 manifest `runtime.allow_audio`/`muted` 策略和更长真实音视频码流覆盖。
