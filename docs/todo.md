...
- [x] 将 H.265 direct decode 从首帧扩展到首个真实多帧 direct smoke：新增 `--decode-h265-ready-prefix N` 和 `h265_ready_prefix_decode` telemetry，把 ready-prefix AU 按 Vulkan bitstream offset/size alignment 顺序写入 `VIDEO_DECODE_SRC_KHR` buffer，录制连续 `vkCmdDecodeVideoKHR`，并 readback 最后一帧 NV12。2026-06-21 在 `WAYLAND_DISPLAY=wayland-1`、H.265 Main 3840x2160@240 short-GOP 源上验证 2-frame IDR+P decode，AU1 引用 AU0/slot0，final readback layer1 的 Y/UV unique=198/248、hash=6114663086929905156/6011523946105094791。当前 8-frame submit 命令可完成但最后 repeated-IDR/P readback 会退化为单值，下一步要补 IDR DPB reset/slot 生命周期后再把完整 ready-prefix 作为通过条件。
- [ ] 将 H.265 direct decode 从首帧离屏 sampling 扩展为连续帧 decode/display 和首个安全可见 smoke，并补 frame pacing/queue 同步/释放 telemetry。
