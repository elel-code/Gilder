...
- [x] 推进 direct H.264 到 session/resource/bitstream gate：`--video-codec h264` 映射到 H.264 High 8-bit progressive profile，新增 `scripts/native-vulkan-h264-bitstream-smoke.sh`，通过 `qtdemux ! h264parse ! appsink` 输出 `stream-format=byte-stream, alignment=au`，验证 SPS/PPS/IDR NAL、NV12 resource image 和 `VIDEO_DECODE_SRC_KHR` bitstream upload。2026-06-21 真实 `WAYLAND_DISPLAY=wayland-1` 证据 `/tmp/gilder-vulkan-h264-bitstream.Pbeh9g`：`requested_codec=h264-high-8`、`h264_parameter_sets_present=true`、`mapped_write_bytes=52450`、`session_memory_bytes=2215936`；H.264 SPS/PPS -> Vulkan STD session parameters 和可见 decode/present 仍是下一步。
...
- [ ] 将 H.265/AV1 encoded frontend 从 ready-prefix 文件 spool 推进到连续 demux/parser ring producer：metadata ring 记录 PTS/duration/slice或tile offset/reference info，decode fence/timeline 释放 range；目标是支持连续播放、音频 clock 和 AV1/H.265 共用码流输入层，而不是长期依赖 ready-prefix window。
...
- [ ] 实现 NVIDIA native Vulkan Video decode path：优先 H.265 main-8 或 AV1 main-8，demux/parser 供码流、codec parameters 和音频/时钟，Vulkan Video 负责 session parameters 和 `vkCmdDecodeVideoKHR`，替代当前 `CUDAMemory -> CUDA copy -> Vulkan` fallback；H.264 已有 High 8-bit session/resource/bitstream gate，后续只承诺驱动 level 5.2 覆盖范围内的源。
