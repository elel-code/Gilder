...
- [x] 将 H.264/H.265/AV1 session/resource/submit gate 收敛到 Vulkanalia：当前入口是 `--probe-vulkanalia-video-session` 和 Vulkanalia ready-prefix video smoke，FFmpeg 是 codec/DPB/submit order 第一参考，GStreamer 只作为 demux/parser/audio frontend。
- [x] 历史中间阶段曾将 H.264/H.265 visible direct smoke 和 CLI 默认输入切到 streaming queue，并停止维护 ready-prefix spool 入口：当时可见 video runtime 不再保留长 AU payload，剩余 H.264 4K/240 缺口主要落在 codec capability、present pacing/同步或驱动路径，不是 packet retention。当前这些 CLI/input 模式已退休，统一到 Vulkanalia owned upload buffer 路线。
...
- [ ] 将 AV1 Main8/Main10 推进到 direct Vulkan Video 任意入口连续可见：当前统一入口为
  `--run-vulkanalia-ready-prefix-video --video-codec av1|av1-main-10`，脚本是
