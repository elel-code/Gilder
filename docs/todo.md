...
- [ ] 继续推进 audio/clock 主线路：H.264/H.265/AV1 ready-prefix runtime 已共用
  ...
  `audio_sample_stale_count=0`、`audio_video_master_clock_drift_abs_max_ns=853202`；下一步补
  H.265/AV1 真实源 gate，然后把现有 target-fps pacer 切成
  video-clock/audio-clock 可选 master，最后接实际音频输出/静音策略。
