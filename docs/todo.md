...
- [x] 将 Vulkanalia H.265 retained decoded image 推进到 graphics sampling 资源边界：
  新增 `vulkanalia_backend/render_present.rs`，在 video+present logical device 上查询并启用
  `samplerYcbcrConversion`，为 retained NV12/P010 image 创建
  `VkSamplerYcbcrConversion`、immutable YCbCr sampler、converted 2D image view 和
  combined-image-sampler descriptor set；同时记录 split queue family 下的
  sync2 release/acquire + dynamic rendering draw 顺序。真实 Wayland `background`
  证据：Main8 `sampler.route=decoded-image-ycbcr-sampler-present-resource`、
  `picture_format=G8_B8R8_2PLANE_420_UNORM`、`retained_submitted=240`；Main10/P010
  `picture_format=G10X6_B10X6R10X6_2PLANE_420_UNORM_3PACK16`、`retained_submitted=240`。
