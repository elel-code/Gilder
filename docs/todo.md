...
- [ ] 将 AV1 hidden decode 同步从单 command buffer 串行等待推进到 slot ownership + command-buffer/timeline ring：2026-06-23 真实 `HDMI-A-1` 4K/240 arbitrary-entry Main8 evidence `/tmp/gilder-av1-main8-hidden-handoff-readback` 显示上一轮“immediate show-existing semaphore handoff”没有命中，`av1_hidden_decode_async_handoff_count=0`、`av1_hidden_decode_queue_wait_count=238`、`average_present_fps=201.318`，说明该源 hidden decode 与 show-existing 不是稳定相邻形态。本轮已先把 hidden decode 默认同步从 `vkQueueWaitIdle` 缩到 per-submit fence wait，并输出 `av1_hidden_decode_fence_wait_*` 和 `bitstream_ring_allocation_count`；Main8 readback `/tmp/gilder-av1-main8-hidden-fence-readback` 与 Main10 readback `/tmp/gilder-av1-main10-hidden-fence-readback` 都保持可见正确且 `queue_wait_count=0`，Main8 长跑 `/tmp/gilder-av1-main8-hidden-fence-4k240-performance` 为 `average_present_fps=209.958`、CPU `15.03%`、`RSS/PSS/USS/Private_Dirty max=108732/95546/90860/30652 KiB`。下一步是按 DPB/output slot 记录完成序号、layout ownership 和 bitstream range lifetime，再启用多 video command buffer/timeline semaphore，避免跨 present layout 切换造成闪烁。
