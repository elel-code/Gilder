...
- [ ] 将 AV1 encoded frontend 也从 ready-prefix/first-frame 文件采样推进到连续 demux/parser ring producer，并接入共用 streaming input layer：metadata ring 记录 PTS/duration/slice或tile offset/reference info，decode fence/timeline 释放 range；目标是从任意合法入口点逐步建立 DPB/reference 状态并支持连续播放、音频 clock，而不是长期依赖 reference-ready ready-prefix window。
