...
- [ ] 为 H.265/AV1 main-10 direct path 补 10-bit 2-plane 420 sampled decode/present path；当前 session/resource/bitstream gate 已确认 `G10X6_B10X6R10X6_2PLANE_420_UNORM_3PACK16` 可用于 Vulkan Video resource image，GStreamer appsink visible importer 已支持 P010 sampling 和 `P010_10LE` 可见 smoke，但 direct Vulkan Video Main10 仍需参数化 P010 resource sampling、plane view/readback size、DPB 和 `vkCmdDecodeVideoKHR` 路径。2026-06-22 回归：H.265 Main10 `/tmp/gilder-vulkan-h265-main10-present-worker-fixed` 通过 session/resource/bitstream gate；AV1 Main10 `/tmp/gilder-vulkan-av1-bitstream-present-worker-10-fixed3` 通过 sequence/session/frame-header/tile-layout telemetry gate。
