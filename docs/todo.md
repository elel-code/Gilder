...
- [ ] 继续推进 audio/clock 主线路：H.264/H.265/AV1 ready-prefix runtime 已共用
  ...
  默认仍是 `clock-only`，避免壁纸静音策略回退。H.264/H.265/AV1 ready-prefix smoke
  已接入 `--audio-output` 参数和 sink-count gate；短 H.264 脚本 gate
  ...
  manifest `muted` 规划输出策略：muted 仍为 `clock-only`，unmuted 规划为 `auto`；下一步是把
  该 opt-in 输出从显式 smoke 参数推进到真实 manifest `runtime.allow_audio`/`muted` 运行时选择。
