...
- 后续 adaptive system monitor 应作为用户可选策略层，默认关闭。开启后可以采样 CPU、
  GPU、内存压力、温度、电源和帧行为，并把结果合入 `decide_performance`，但只能提出更
  保守的决策，例如降低 FPS、暂停非焦点输出或停止高负载动态壁纸，不能覆盖用户暂停、
  fullscreen pause、battery pause 等更强策略。该策略需要配置阈值、冷却时间、每输出
  开关和全局 kill switch，并在 `status`/telemetry 中报告触发原因，方便用户审计。
