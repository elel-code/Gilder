...
- web 壁纸是单独的浏览器 runtime，不是 video sink 的延伸。第一阶段目标是
  `gilder-web-renderer` helper：内部实现可在 WPE WebKit、CEF/Ozone Wayland、WebKitGTK
  或等价 C/GObject 绑定之间替换，主 daemon 只通过 IPC 下发 root/index、属性、mute、
  pause、FPS/可见性和权限策略。GTK-rs 不能进入 daemon/core/native Vulkan 依赖；如果
  WebKitGTK 被用作临时 helper，只能停留在隔离进程内部，并通过 frame/texture handoff
  与主 renderer 交互。
