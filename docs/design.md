...
淘汰计数、静态大图运行时降采样缓存的生成/复用/淘汰计数、计划层静态图/poster/slideshow 图片资源数量和源文件字节 footprint、GTK renderer
当前 static CSS provider/slideshow surface 指向的源资源引用数、去重资源数和字节 footprint，以及渲染器同步更新
