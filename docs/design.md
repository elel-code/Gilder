...
5. 在 native Wayland host 中管理 surface 生命周期，在后台线程、FFmpeg frontend 或
...
- `generic-wayland`：只依赖 Wayland/session 可见信息和验证覆盖，作为默认兜底路径。
...
throttling、fullscreen pause 和 output-hidden pause；未设置时使用合成器适配器
采集到的真实输出状态。
...
并行推进，路线和停止/成功条件记录在 `docs/vulkan-migration.md`；已删除的旧 renderer
路径不再作为当前架构、脚本或验证目标保留。
...
- 当前主线是 `native-vulkan-video`：FFmpeg 做 demux/bitstream filter/access-unit
  前端，native Vulkan 负责 GPU decode/render/present。
- 有效路线是 bitstream-native-decode：FFmpeg 只到 encoded access-unit，
  H.264/H.265/AV1 解码由 Gilder native Vulkan Video 负责。
- daemon 会为 video entry 生成 `render_sync.video_plans`，包含 source、poster、
  loop、muted、fit、start offset 和性能策略合成后的目标 FPS。
- 如果 video entry 提供 poster，或 manifest 的 `preview.poster` 可用，daemon 只把它
  保留在 `video_plans[].poster` 元数据中，不再生成静态 poster surface fallback。
- 默认音频被丢弃。可见视频的 FPS/pacing 由 native Vulkan present loop 控制。
- 支持 MP4/H.264、H.265 和 AV1，实际 demux/parser/decoder 支持由 FFmpeg、Vulkan
  Video capability 和 GPU driver 共同决定。
- 循环、静音、音频丢弃、最大 FPS、poster、空闲暂停必须是 manifest 中的显式策略。
- 解码和播放控制不阻塞 IPC/desktop refresh 线程。

Native Vulkan Video 方向：

- FFmpeg frontend 只负责 container demux、bitstream filter 和 access-unit 生命周期；
  不能引入 decoded frame handoff 或 CPU-side frame queue。
- Vulkan decode/render/present 负责 H.264/H.265/AV1 的全链路 GPU 路线，descriptor
  model 必须保持 `VK_EXT_descriptor_heap`，不得引回 descriptor set。
- 性能证据必须报告 decode queue、DPB/image 层数、bitstream ring、present FPS、frame
  pacing、Private_Dirty、PSS/USS 和 GPU memory；真实 presentation 证据来自 Vulkan
  present telemetry、Wayland frame callback 或 `wp_presentation`。
...
- static image、slideshow、scene-lite、shader 和 video 都应收敛到 native Vulkan backend。
  FFmpeg 只提供 demux/bitstream/audio/clock 语义，不能接管 decode/render/present。
- web 壁纸是单独的浏览器 runtime，不是 video sink 的延伸。第一阶段目标是
  `gilder-web-renderer` helper：内部实现可在 WPE/CEF/Ozone Wayland 或等价
  sandboxed browser provider 之间替换，主 daemon 只通过 IPC 下发 root/index、属性、
  mute、pause、FPS/可见性和权限策略，并通过 frame/texture handoff 与主 renderer 交互。
...
如果没有显式 variant，计划阶段会使用 desktop snapshot 中的输出尺寸与 scale 自动选择最小的
...
- adaptive system monitor 是用户可选策略层，默认关闭。开启后会采样 Linux PSI
  ...
  `status`/telemetry 中报告当前采样、触发原因和 adaptive 动作，方便用户审计。native
  Vulkan video 的详细 decode/present、FPS 和 memory 证据由专用 smoke/runtime summary
  输出。
...
native Wayland/Vulkan/FFmpeg 前端反复创建中间态 surface、pipeline 或 runtime snapshot。
...
`[video].decoder` 由 native Vulkan video 路线消费。当前主线优先 native Vulkan Video
硬解；`hardware-required` 必须在 capability 不满足时失败，不能静默回退到 CPU decoded
frame 路线。
