...
有效证据必须来自 Vulkanalia runtime JSON 中的 owned upload buffer 模型：
`decode.bitstream_buffer_model=ready-prefix-owned-upload-buffer`、
...
`streaming-queue`、spool 或 bitstream ring 文本只表示 2026-06-21/22 的历史 baseline。
...
- H.264 visible direct path 的 2026-06-21 历史维护目标曾是 streaming packet queue：
  ...
  runtime 证据以 owned upload buffer 模型为准。历史 GStreamer appsink AU 会进入 bounded packet
...
- 旧 ash `probe-video-session` 链路已退役：它曾证明 session/resource image/bitstream
  buffer、H.264/H.265/AV1 参数集转换、首帧/ready-prefix submit、host readback 和离屏
...
- AV1 direct 已跨过上述 show-existing/reference planner 阻塞，接入真实可见 runtime：
  当前统一入口是 `--run-vulkanalia-ready-prefix-video --video-codec av1|av1-main-10`，
  脚本是 `scripts/native-vulkan-av1-ready-prefix-video-smoke.sh`。runtime 支持 AV1 Main8/Main10、
