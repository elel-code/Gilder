...
dirty 做底层压榨；下一阶段优先扩展和稳定其他壁纸类型，同时把新增能力写成可以迁移到
纯 Vulkan 后端的形状。
...
下一阶段重点从 video active memory 转到类型覆盖和稳定性：
...
这些工作不能等纯 Vulkan 完成后才做。要求是新增 runtime 的接口和数据模型保持后端无关，
让当前 GTK/wgpu 实现和未来 Vulkan 实现可以共享同一份 manifest、render plan、属性系统和
验证门槛。
...
- 只有当 static/shader/scene 基本能力稳定后，再把 video interop 放进 spike。
